...
instance Monoid t => Copointed (EventG t) where
  -- E a -> F (R a) -> R a -> a
  copoint = extract . extract . eFuture
...
monotonicity :: (Show a, Arbitrary a, Arbitrary t
                ,Num t, Show t, Ord t, Bounded t, Ord t', Bounded t')
...
simulEventOrder :: ( Arbitrary t, Num t, Show t, Ord t, Bounded t
                   , Arbitrary t', Num t', Show t', Ord t', Bounded t'
